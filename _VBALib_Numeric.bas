Attribute VB_Name = "_VBALib_Numeric"
Option Explicit

Enum NumberPrintReturnType 'Used w/ NumberToPrint
    ReturnNumber = 1
    ReturnFormat = 2
End Enum

Function NumberToPrint(Number As Variant, ReturnType As NumberPrintReturnType, ShowCommas As Boolean) As Variant
    'Returns Number or Number Format (if numeric), Text or Text Format (not numberic), Error (o.w.) given Number
    Dim NumAfterDecimal As Double, ConvertedText As String, ShowDec As String, TextFormat As String
    If IsNumeric(Number) = False Then GoTo NonNumeric                  'DON'T ADD PRECISION WHEN THERE ISN'T ANY
...
Function LogX(Number As Double, Optional Base As Double = 10) As Variant
    'Converts a number to LogX form, Log10 by default
    On Error GoTo IsError
...
' [] 
'----------------------------------------------------------------------------------------------------
Function StudentTText_UnequalVar(ByVal ControlMean As Double, ByVal ControlSD As Double, ByVal ControlN As Integer, ByVal DoseMean As Double, ByVal DoseSD As Double, ByVal DoseN As Integer) As Double
    'Calculates a student t-test with unequal sample size and unequal variance, using a mean and standard deviation for two distributions.
    'In : ControlMean As Double, ControlSD As Double, ControlN As Integer, DoseMean As Double, DoseSD As Double, DoseN As Integer
...
End Function
Function StudentTText_EqualVar(ByVal ControlMean As Double, ByVal ControlSD As Double, ByVal ControlN As Integer, ByVal DoseMean As Double, ByVal DoseSD As Double, ByVal DoseN As Integer) As Double
    'Calculates a student t-test with equal sample size and equal variance, using a mean and standard deviation for two distributions
    'In : ControlMean As Double, ControlSD As Double, ControlN As Integer, DoseMean As Double, DoseSD As Double, DoseN As Integer
    'Out: Double T-test p-value, or -999 if error
...
End Function
Function FishersExactText(ByVal A1 As Long, ByVal B1 As Long, ByVal A2 As Long, ByVal B2 As Long) As Variant
    'Calculates a pair-wise significance test using fisher's exact test method Modified to calculate in log-space to allow for much larger matrices (tested w/ integer value of 12,000+)
    'In : A1 As Long, B1 As Long, A2 As Long, B2 As Long                    'Out: FishersExactTest as Double
    ...
    LogMatrix(3, 3) = LogXFactorial(A1 + A2 + B1 + B2, 10)     ' added/subtracted rather than multiplied/divided becase in logspace
    FishersExactText = 10 ^ (LogMatrix(1, 3) + LogMatrix(2, 3) + (LogMatrix(3, 1) + LogMatrix(3, 2)) - (LogMatrix(3, 3) + LogMatrix(1, 1) + LogMatrix(1, 2) + LogMatrix(2, 1) + LogMatrix(2, 2)))
    Exit Function
IsError:
