Attribute VB_Name = "_VBALib_Examples"
Option Explicit
...
Sub ExampleCode()
    'MSExcel Controls - use Excel_[On,Off] or Excel_Toggle
    With Application
        .ScreenUpdating = True 'False        'SCREEN UPDATING
        .Calculation = xlManual              'SHEET CALCULATION
        .Calculation = xlAutomatic
        .StatusBar = "Add text here..."      'STATUS BAR AT BOTTOM
        .StatusBar = False 'True
        .DisplayAlerts = True 'False         'DISPLAY ALERTS
    ...
    PasteRange.Value = CopyRange.Value  '(values only)
    .Copy: .PasteSpecial ...    '(PasteSpecial=BETTER) 
    ...
    'USE IIf/IIf() (FASTER EVAL)
    IIf IsError(Application.Match(SearchString, Range("SearchList"), False)), False, True
    RetVal = IIf(Application.Match(SearchString, Range("SearchList"), False), False, True)
