...
'Dim appWd As Word.Application: Set appWd = CreateObject("Word.Application")
Public Sub MSWord_FmtSel(Optional sFont As String = "Calibri", Optional lSize As Long=12,Optional bBold As Boolean=True,Optional bItalic As Boolean = True)
    'assumes Selection, no options required for default format shown above - needs work
    Dim appWd As Word.Application: Set appWd = Word.Application
    With appWd.Selection
        .Style = "Normal"
        With .Font
            .Name = sFont: .Size = lSize
            .Bold = bBold: .Italic = bItalic
        End With '.Font
        .ParagraphFormat.Alignment = wdAlignParagraphLeft
    End With 'appWd.Selection
End Sub 'MSWord_FmtSel
